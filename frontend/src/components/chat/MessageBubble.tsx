...
export function MessageBubble({ message, galleryOffset, onOpenContextMenu }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex w-full animate-slide-up ${outgoing ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        <div className="absolute right-2 top-2 z-20 flex items-center gap-1 rounded-full border border-border/40 bg-header/95 p-0.5 text-secondary opacity-0 shadow-sm transition-opacity group-hover/message-bubble:opacity-100 focus-within:opacity-100">
          <button
            type="button"
        ...
        {videos.length > 0 && (
          <div className="mb-2 flex flex-col gap-1.5">
            {videos.map((video, idx) => {
              ...
              const externalUrl = video.url ?? src
              return (
                <div key={idx} className="group relative w-full max-w-[320px]">
                  <video
                    src={src}
                    controls
                    preload="metadata"
                    className="w-full max-h-80 rounded-lg border border-border/20 bg-black"
                  />
                  {externalUrl && (
                    <button
                      type="button"
                      onClick={() => openExternal(externalUrl)}
                      title={t('chat.openExternalPlayer')}
