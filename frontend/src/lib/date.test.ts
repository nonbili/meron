...
describe('formatThreadDate', () => {
  ...
  it('formats yesterday as "Yesterday"', () => {
    expect(formatThreadDate(sec(new Date(2026, 5, 9, 9, 0)))).toBe('Yesterday')
  })

  it('formats earlier this week as a weekday name', () => {
    // Monday 2026-06-08, two days before the fixed "now".
    expect(formatThreadDate(sec(new Date(2026, 5, 8, 9, 0)))).toBe('Monday')
  })
