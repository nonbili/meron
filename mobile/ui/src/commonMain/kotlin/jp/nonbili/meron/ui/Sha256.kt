...
/** Pure-Kotlin SHA-256 so it works identically on all targets without platform crypto. */
...
/** Lowercase hex SHA-256 (e.g. Gravatar identifiers, which accept SHA-256). */
fun sha256Hex(value: String): String =
    sha256(value.encodeToByteArray()).joinToString("") {
        val v = it.toInt() and 0xff
        val hex = v.toString(16)
        if (hex.length == 1) "0$hex" else hex
    }

/** PKCE S256 code challenge for an authorization-code verifier. */
