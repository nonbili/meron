...
class MailViewModel(
    ...
    fun selectThread(threadId: String, messages: List<MessageBody>) {
    ...
    fun setThreadStarred(threadId: String, starred: Boolean) {
        updateThread(threadId) { it.copy(starred = starred) }
    }

    fun setThreadRead(threadId: String, read: Boolean) {
    ...
    private fun updateThread(threadId: String, update: (ThreadSummary) -> ThreadSummary) {
        val existed = state.threads.any { it.id == threadId }
        require(existed) { "Unknown thread: $threadId" }
